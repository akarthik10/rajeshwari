...
{?>
 <h1><?php 
		...
		?>
        	<h1>
        	<?php echo Yii::t('Batch','Manage Batch'); ?>
            </h1>
		<?php
        }
        ?></h1>
 ...
 <?php    $batch=Batches::model()->findByAttributes(array('id'=>$_REQUEST['id'])); 
    ...
    <?php if((Yii::app()->controller->id=='batches' and (Yii::app()->controller->action->id=='batchstudents' or Yii::app()->controller->action->id=='settings')) or (Yii::app()->controller->id=='subject' and Yii::app()->controller->action->id=='index') or (Yii::app()->controller->id=='weekdays' and (Yii::app()->controller->action->id=='timetable' or Yii::app()->controller->action->id=='index')) or (Yii::app()->controller->id=='classTiming' and Yii::app()->controller->action->id=='index') or (Yii::app()->controller->id=='studentAttentance' and Yii::app()->controller->action->id=='index') or (Yii::app()->controller->id=='gradingLevels' and Yii::app()->controller->action->id=='index') or (Yii::app()->controller->id=='exam' and Yii::app()->controller->action->id=='index'))
			...
			echo CHtml::ajaxLink(Yii::t('Batch','Change Batch'),array('/site/explorer','widget'=>'2','rurl'=>$rurl[0]),array('update'=>'#explorer_handler'),array('id'=>'explorer_change','class'=>'sb_but','style'=>'top:-40px; right:40px;')); ?>
           
<?php 
			}else
			{
				echo CHtml::ajaxLink(Yii::t('Batch','Change Batch'),array('/site/explorer','widget'=>'2','rurl'=>'courses/batches/batchstudents'),array('update'=>'#explorer_handler'),array('id'=>'explorer_change','class'=>'sb_but','style'=>'top:-40px; right:40px;'));
			}?>
        ...
        <div style="position:absolute; top:27px; right:100px; ">
        	<div id="tabdrop" class="act_but"><?php echo Yii::t('Batch','Actions'); ?></div>
            <div class="act_drop" id="tabdropx">
				<div class="but_bg_outer"></div><div class="but_bg"><div id="1" class="act_but_hover">Actions</div></div>
                <ul>
					<li class="addstud"><?php echo CHtml::link(Yii::t('Batch','Add Student').'<span>'.Yii::t('for add new student').'</span>', array('/students/students/create','bid'=>$_REQUEST['id'])) ?></li>
					<li class="newsub"><?php echo CHtml::link(Yii::t('Batch','New Subject').'<span>'.Yii::t('for add new subject').'</span>', array('#'),array('id'=>'add_subject-name-side')) ?></li>
                   	<li class="mark"><?php echo CHtml::link(Yii::t('Batch','Mark Attendance').'<span>'.Yii::t('for add leave').'</span>', array('/courses/studentAttentance','id'=>$_REQUEST['id'])) ?></li>
                    <li class="mark"><?php echo CHtml::link(Yii::t('Batch','Add Elective').'<span>'.Yii::t('for add new subjects').'</span>', array('batches/elective','id'=>$_REQUEST['id'])) ?></li>
				   	<li class="promote"><?php echo CHtml::link(Yii::t('Batch','Promote Batch').'<span>'.Yii::t('for promote a batch').'</span>', array('batches/promote','id'=>$_REQUEST['id'])) ?></li>
    ...
    <?php
		...
		if($week==NULL)
		{
			$weeknot='<div class="cbi_gray">'.Yii::t('Batch','Default Values Selected').'</div>';
			$weeklink=CHtml::link(Yii::t('Batch','Define Now'), array('/courses/weekdays','id'=>$_REQUEST['id']));
			$allgreen=0;
		}
		else
		{
			$weeknot='<div class="cbi_green">'.Yii::t('Batch','Weekdays defined').'</div>';
			$weeklink='';
		...
		if($timing==NULL)
		{
				$timingnot='<div class="cbi_red">'.Yii::t('Batch','No class timings defined').'</div>';
				$ttabnot='<div class="cbi_gray">'.Yii::t('Batch','Timetable not created').'</div>';
		...
		{
				  $timingnot='<div class="cbi_green">'.Yii::t('Batch','Class Timings Defined').'</div>';
				  $timinglink='';
			  
			  $ttab=TimetableEntries::model()->findByAttributes(array('batch_id'=>$batch->id));
			  if($ttab==NULL)
			  {
				  $ttabnot='<div class="cbi_red">'.Yii::t('Batch','Timetable not created').'</div>';
				  $ttablink=CHtml::link(Yii::t('Batch','Create Now'), array('weekdays/timetable','id'=>$_REQUEST['id']));
				  $allgreen=0;
			  }
			  else
			  {
				  $ttabnot='<div class="cbi_green">'.Yii::t('Batch','Timetable Created').'</div>';
				  $ttablink='';
			  }
		}
    ...
    <div class="cb_info_bx">
        ...
        <div class="cbi_ibx cbi_ico3" style="border-left:none">
        	<h3><?php echo Yii::t('Batch','Active Students'); ?></h3>
            <?php echo $studnot; ?>
            <?php echo $studlink; ?>
        </div>
        
        <div class="cbi_ibx cbi_ico5">
        	<h3><?php echo Yii::t('Batch','Subjects'); ?></h3>
            <?php echo $subnot; ?>
            <?php echo $sublink; ?>
        </div>
        
    	<div class="cbi_ibx cbi_ico1">
        	<h3><?php echo Yii::t('Batch','Weekdays'); ?></h3>
            <?php echo $weeknot; ?>
            <?php echo $weeklink; ?>
        ...
        <div class="cbi_ibx cbi_ico2">
        	<h3><?php echo Yii::t('Batch','Class Timings'); ?></h3>
            <?php echo $timingnot; ?>
            <?php echo $timinglink; ?>
        </div>
        
        <div class="cbi_ibx cbi_ico4" style="border-right:none">
        	<h3><?php echo Yii::t('Batch','Timetable'); ?></h3>
            <?php echo $ttabnot; ?>
            <?php echo $ttablink; ?>
        </div>
